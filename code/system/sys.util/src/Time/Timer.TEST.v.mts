...
describe('timer', () => {
  ...
  it('reports elapsed days', () => {
    ...
    expect(time.timer(start).elapsed.day).to.eql(4.7 || 4.8);
    expect(time.timer(start, { round: 1 }).elapsed.day).to.eql(4.7 || 4.8); // NB: hack for variation between MacOS and Linux.

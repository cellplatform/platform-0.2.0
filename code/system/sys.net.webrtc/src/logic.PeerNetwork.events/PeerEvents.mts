...
export function PeerEvents(
  ...
  const bus$ = bus.$.pipe(
    ...
    rx.filter(EventNamespace.is.peer.base),
    rx.map((e) => e as t.PeerEvent),
  );
  ...
  const connection = (self: t.PeerId, remote: t.PeerId) => {
    ...
    const connected$ = rx
      ...
      .pipe(rx.filter((e) => e.self === self && e.remote === remote));

...
The system (platform) lives as an "open commons" shared resource for the world. As such the core modules are [open source](https://en.wikipedia.org/wiki/Open-source_software) (OSS) and all up-stream dependencies conform with transitively equivalent OSS licences. In the case of this repo the baseline is the [MIT Licence](LICENSE). 

To run a licence analysis and validate the depenency graph run:
